...
if __name__ == "__main__":
    ...
    print("Select the most common SNPs from merged version of (Familial and NINDS1) dataset and check those SNPs quality for getting the accuracy - Both datasets are balanced:")    
    
    # reading most common SNPs
    SNPsList = []    
    f1 = open('/scratch/fs2/usefi/pd/datasets/All_Aproaches/Approach4/Part_A/FamilyAndNINDS1/CV/mostCommon.txt', 'r')
    for line1 in f1:
    ...
    print("Reading FamilyNINDS1(MergedAndBalancedWithID).csv ...")
    df= pd.read_csv(path + "FamilyNINDS1(MergedAndBalancedWithID).csv")
    ...
    x_train = fold1234.iloc[:, fold1234.columns.isin(SNPsList)]
    y_train = fold1234.Label
    x_test = fold5.iloc[:, fold5.columns.isin(SNPsList)]
    y_test = fold5.Label
    ...
    x_train = fold1235.iloc[:, fold1235.columns.isin(SNPsList)]
    y_train = fold1235.Label
    x_test = fold4.iloc[:, fold4.columns.isin(SNPsList)]
    y_test = fold4.Label
    ...
    x_train = fold1245.iloc[:, fold1245.columns.isin(SNPsList)]
    y_train = fold1245.Label
    x_test = fold3.iloc[:, fold3.columns.isin(SNPsList)]
    y_test = fold3.Label
    ...
    x_train = fold1345.iloc[:, fold1345.columns.isin(SNPsList)]
    y_train = fold1345.Label
    x_test = fold2.iloc[:, fold2.columns.isin(SNPsList)]
    y_test = fold2.Label
    ...
    x_train = fold2345.iloc[:, fold2345.columns.isin(SNPsList)]
    y_train = fold2345.Label
    x_test = fold1.iloc[:, fold1.columns.isin(SNPsList)]
    y_test = fold1.Label
